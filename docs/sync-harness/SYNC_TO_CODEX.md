...
- **스킬·에이전트 이름**: `plan-dev`, `implement-dev`, `fix-dev`, `test-dev`, `review-code`, `commit-code`, `request-merge`, `dev-loop` (modes `light`/`full`/`noreview`); persona `planner`, `implementer`, `security-reviewer`, `reliability-reviewer`, `maintainability-reviewer`, `senior-generalist-reviewer`.
...
- `planner`, `implementer`, reviewer personas처럼 custom agent 이름을 지정해 spawn할 때는 항상 `fork_turns="none"`을 명시한다. Self-contained prompt를 넘기더라도 이 인자를 생략하지 않는다.
