...
이 문서는 Codex 기준으로 작성된 Agent Skill을 Cursor에 맞게 옮길 때 적용하는 일반 점검표다. 특정 스킬에 묶이지 않도록 작성하며, 스킬이 변경되거나 새로 추가될 때도 같은 기준으로 검사한다. 소스는 `skills/codex/<skill>`, 대상은 `skills/cursor/<skill>`다.
...
- `review-code`의 네 persona 이름은 그대로 유지한다: `security_reviewer`, `reliability_reviewer`, `maintainability_reviewer`, `senior_generalist_reviewer`. "Codex custom agent" → "Cursor custom subagent (Task tool)"로 표현만 바꾼다. 병렬 dispatch는 "한 턴에 여러 Task 호출"로 적는다.
...
## 7. Define the reviewer personas as Cursor subagents

`review-code`가 위임하는 네 persona는 Cursor custom subagent로 별도 정의해야 한다(스킬 본문이 이름으로 참조하기 때문). skill 본문 마이그레이션과 짝을 이루는 작업이다.

- harness 소스는 플랫폼별 디렉토리에 둔다. 예: `agents/cursor/*.md` (claude=`*.md`, codex=`*.toml`와 평행). 설치 대상은 전역이면 `~/.cursor/agents/`, 프로젝트 범위면 `.cursor/agents/`.
- Codex TOML(`name`, `description`, `developer_instructions`)을 Cursor Markdown(YAML frontmatter + 본문)으로 변환한다. frontmatter 최소 키는 `name`, `description`이고, 필요 시 `model: inherit`, `readonly: true`, `is_background`를 둔다.
- 리뷰어는 읽기 전용이므로 `readonly: true`로 두고, 본문에 "no edits, no commits"를 남긴다.
- `name`은 skill 본문의 참조와 일치해야 한다. 사용자가 underscore 이름(`security_reviewer` 등)을 쓰기로 했으므로 그대로 유지한다. 단 Cursor 문서는 hyphen 네이밍을 권장하므로, underscore 이름이 인식되지 않으면 skill 본문과 함께 hyphen으로 통일한다.
- 파일명과 `name`을 맞추고, Cursor 내장 이름(`Explore`, `Bash`, `Browser`)과 충돌하는 custom `name`은 피한다.
...
- 설치/배포 배선(`scripts/apply-to-global.sh`에 cursor 분기 추가, `~/.cursor/skills` 동기화)과 hooks 변환은 이 점검표에 포함하지 않는다. skill 본문(A/C/D) 마이그레이션에 집중한다.
