#!/usr/bin/env bash
# beforeShellExecution hook: block `find` invocations doing file/code search
# (i.e. using -name/-iname/-path/-ipath/-regex/-iregex). Suggests `fd` with
# translation hints via agent_message. Metadata-only `find` (e.g. -mtime,
# -size, -perm) is allowed because fd does not cover those.
#
# Requires: jq
set -euo pipefail
...
EOF
  ...
  jq -n --arg am "$msg" '{permission:"deny", user_message:"Use fd instead of find for file search.", agent_message:$am}'
  exit 0
fi
