#!/usr/bin/env bash
# preToolUse hook: verify git user.email matches the repo type (personal vs
# work) before `git commit` or `git push`.
#
...
EOF
  ...
  jq -n --arg msg "$message" \
    '{permission:"deny",agent_message:$msg,user_message:"Git identity mismatch — commit/push blocked."}'
  exit 0
fi
