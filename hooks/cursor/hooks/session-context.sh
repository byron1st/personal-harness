#!/usr/bin/env bash
# sessionStart hook: classify the repository as personal or work, and inject
# that plus the global harness instructions as session-scoped context.
#
# Two differences from the Claude variant:
#   1. sessionStart has no `cwd`; the repo root comes from `workspace_roots[0]`.
#   2. Cursor has no user-global instructions file. `~/.cursor/AGENTS.md` is
#      installed by apply-to-cursor.sh and Cursor never reads it — this hook
#      does, and appends it to additional_context. That is what gives Cursor
#      the equivalent of ~/.claude/CLAUDE.md.
#
# Env vars used: WORK_GITLAB_HOST, WORK_GIT_EMAIL, WORK_GIT_NAME,
#                PERSONAL_GIT_EMAIL, PERSONAL_GIT_NAME
# Requires: jq, git
set -euo pipefail

input=$(cat)
cwd=$(echo "$input" | jq -r '.workspace_roots[0] // ""')

global_instructions=""
if [[ -f "$HOME/.cursor/AGENTS.md" ]]; then
  global_instructions=$(cat "$HOME/.cursor/AGENTS.md")
fi

emit() {
  jq -n --arg ctx "$1" '{additional_context:$ctx}'
}

if [[ -z "$cwd" ]]; then
  # No workspace: the repo classification is meaningless, but the global
  # instructions still apply.
  [[ -n "$global_instructions" ]] && emit "$global_instructions"
  exit 0
fi

remote=""
branch=""
actual_email=""
if git -C "$cwd" rev-parse --git-dir >/dev/null 2>&1; then
  remote=$(git -C "$cwd" remote get-url origin 2>/dev/null || echo "")
  branch=$(git -C "$cwd" branch --show-current 2>/dev/null || echo "")
  actual_email=$(git -C "$cwd" config user.email 2>/dev/null || echo "")
fi

repo_type="personal"
classification_rule="origin remote did not match WORK_GITLAB_HOST"
if [[ -n "${WORK_GITLAB_HOST:-}" && "$remote" == *"$WORK_GITLAB_HOST"* ]]; then
  repo_type="work"
  classification_rule="origin remote matched WORK_GITLAB_HOST"
elif [[ -z "$remote" ]]; then
  classification_rule="no origin remote; defaulted to personal"
elif [[ -z "${WORK_GITLAB_HOST:-}" ]]; then
  classification_rule="WORK_GITLAB_HOST unset; defaulted to personal"
fi

if [[ "$repo_type" == "work" ]]; then
  expected_email="${WORK_GIT_EMAIL:-}"
  expected_name="${WORK_GIT_NAME:-}"
else
  expected_email="${PERSONAL_GIT_EMAIL:-}"
  expected_name="${PERSONAL_GIT_NAME:-}"
fi

context=$(cat <<EOF
Session repository classification:
- repo_type: ${repo_type}
- cwd: ${cwd}
- origin: ${remote:-<none>}
- branch: ${branch:-<none>}
- classification_rule: ${classification_rule}
- expected_commit_identity: ${expected_name:-<unset>} <${expected_email:-<unset>}>
- current_git_email: ${actual_email:-<unset>}

Use repo_type as session-scoped context. If you have not already done so, mention once to the user near the start of the session that this repository was detected as ${repo_type}. When committing or using commit-code, follow the ${repo_type} path: verify git user.name and user.email against the expected ${repo_type} identity, extract a Jira key from the branch only for work repositories, and do not push unless the user explicitly asks or the commit-code run is opening a PR/MR (that path implies push).
EOF
)

if [[ -n "$global_instructions" ]]; then
  context="${context}

---

${global_instructions}"
fi

emit "$context"
