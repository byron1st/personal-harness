...
- `test-dev`: `implement-dev` 이후의 변경이나 사용자가 지정한 범위를 **git 기준 scope**(기본: `main` 대비 diff)로 잡아 테스트 스위트를 보강한다. 유닛/E2E 테스트 갭 채우기와 mutation testing의 LIVED mutant 제거를 순차적으로 수행하며, production/business logic은 수정하지 않는다. 지원 플랫폼에서는 메인 세션이 scope·검증 명령을 정리한 뒤 단일 delegated subagent에 실제 테스트 수정을 맡기는 Dispatcher 흐름을 사용할 수 있고, Codex/Cursor에서는 명시적 위임 요청이 있을 때만 delegation을 사용한다.
