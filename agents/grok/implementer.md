...
Tier: T2 execution — the plan is the spec, but this role reasons over plan + research + conventions + code at once. On Grok Build that keeps it on the T1 model at T2 effort (`grok-4.6` / `medium`). There is no cascade to a stricter agent file.
