...
A completion report is a **thin review overlay** on an AI-written change: what was implemented and why, pointing the reviewer at the diff rather than copying it, and surfacing where to be suspicious. Its spine is per-TODO fulfillment — for each plan TODO, where it landed, what test pins it, which AC it satisfies, and where it diverged. Produced by `implement-dev`, stored under `docs/agents/dev`.
