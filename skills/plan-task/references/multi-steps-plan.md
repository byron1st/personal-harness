...
A multi-steps plan is one **main plan file** plus one or more **sub-plan files**, each describing a single incremental step. The sub-plans are linked from the main plan via Obsidian **wikilinks** (`[[...]]`). Use this format when `plan-task` is operating in **multi-steps** mode (user explicitly requested a multi-step breakdown).
