---
name: fix-dev
description: Fix an error or bug discovered during a review phase without polluting the main session's context. The main session organises the issue into a self-contained brief and dispatches a fresh sub-agent via the Agent tool. Use when the user asks to correct a defect without a new plan. Does NOT commit unless the user explicitly asks for a commit in the same turn.
---
