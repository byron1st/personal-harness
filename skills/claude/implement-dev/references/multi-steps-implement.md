...
Use this reference when `implement-dev` is operating in **multi-steps** mode: a main plan file linking to `-STEP-N` sub-plans, each a complete build-test cycle. The **main session** orchestrates one step at a time: it dispatches each step to a fresh Claude Code sub-agent (`Agent` tool, `subagent_type: general-purpose`) that runs the actual TDD work on a `feature/step-N` branch off `develop`, then handles the per-step review gate and the merge back to `develop`.
...
- `## Conventions` / `## Tech Stack` / `## Architecture Overview` - context every step must respect; pass these to every sub-agent.
...
1. **Dispatch** a Claude Code sub-agent for step N (section 4). Wait for it to return.
2. **Present** the step summary: especially the Red Flags, Open Questions, and `## Manual Verification` items.
3. **Wait** for explicit user approval. If the user requests changes, use a follow-up sub-agent. Re-request approval and do not merge until approved.
4. **Merge** `feature/step-N` into `develop` (section 6), delete the feature branch, run post-merge validation.
5. Move on to step N+1.

Do not start step N+1 until step N has been approved and merged. When using a sub-agent, the main session does not re-read the per-step report file or pull implementation details into its own context; everything it needs is in the sub-agent's returned summary.
...
The full report content lives in the report file; the sub-agent must **not** dump the entire report into its return message.
...
## 5. Per-step work [sub-agent]

The sub-agent performs the following sequence and produces the summary defined in section 4.2.
...
Return the summary defined in section 4.2 as the sub-agent's single final message.
...
If this fails, the merge introduced a regression. Investigate; for non-trivial fixes, use a fresh sub-agent on a `hotfix/step-N` branch. Otherwise fix in the main session.
