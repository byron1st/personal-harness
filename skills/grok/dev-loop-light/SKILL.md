---
name: dev-loop-light
description: "The middle loop — drives an approved single-step plan-dev plan through implement → test → two-axis review → (triage/fix) until READY_TO_COMMIT. Use when the change genuinely wants a review but four axes plus mutation would be overkill; only `maintainability-reviewer` and `senior-generalist-reviewer` run, and mutation testing is out of scope. Drop to `dev-loop-noreview` for ordinary everyday work, escalate to `dev-loop` when the change is genuinely serious or large (security- or reliability-sensitive paths belong there — this variant does not run those two axes). Requires a plan with Acceptance Contract and Authority Boundaries; refuses legacy plans and routes them to plan-dev. Never commits, pushes, or opens PR/MRs."
---
