...
The dispatcher (the main `implement-dev` session) hands the Worker this prompt, replacing placeholders. It fills the verification-command block from `Prepare` step 2 (`$HOME/.grok/scripts/detect-commands.sh` plus any prose-only commands) — passing them is the point: without this block the Worker rediscovers the same commands cold on every dispatch.
...
`pass` = every TODO fulfilled, verification green, and every AC evidenced under `## Evidence` (legacy plans: the `none (legacy plan)` line recorded instead - an unproven AC blocks `pass`). `blocked` = at least one TODO is blocked on a direction-level decision (no further code should be written past that conflict; detail-level obstacles are not blockers). `failed` = verification failed irrecoverably after `implement-dev`'s Error Recovery, or an unexpected hard error. `needs-design-decision` = a `(design-bearing)` fork; the Worker must not spawn consultants (depth 1) — the Dispatcher spawns `plan-consultant` and re-dispatches.
...
- One line when a retry fired (§E'): that the first Worker returned `failed` and whether the second attempt recovered. Never omit this line to keep the summary tidy — it is the metric. On Grok this is **not** a model-tier cascade.
...
## E'. One same-persona retry on a `failed` return (no model cascade)
