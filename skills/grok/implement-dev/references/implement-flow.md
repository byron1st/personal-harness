...
- **Consultable** - a band *between* the two above: both candidate approaches are consistent with the plan, the plan is silent on which, and the wrong one is expensive to reverse (a persisted shape, a public signature, a concurrency model, a boundary other code will grow against). The *what* is not in question, so this is not direction-level; but the choice outlives the TODO, so treating it as ordinary mechanics is how a plan gets silently narrowed. **Only on a TODO tagged `(design-bearing)`**: do **not** spawn `plan-consultant` (Grok depth 1). Return `status: needs-design-decision` with the fork brief so the **Dispatcher** can spawn `plan-consultant`, then resume you with the decision. After resume, record the decision under the TODO's `편차` line and continue. Direction conflicts remain `blocked` for the user.
  ...
  - **A consultant cannot authorize a direction change.** If the answer would contradict the plan's goal / approach / `## Key decisions` / `## Non-goals`, the item was direction-level all along: stop and escalate per the bucket below. Do not use consultation to route around the user.
