---
name: fix-dev
description: Fix an error or bug discovered during a review phase without polluting the main session's context. The main session dispatches a fresh sub-agent via the spawn_subagent tool and requires an explicit decision before direct fallback. Use when the user asks to correct a defect without a new plan. Does NOT commit; leaves the working tree as-is so the user can commit later.
---
...
- **Verification candidates** — known reproduction/targeted lint/test commands and project-required fast gates. Get the declared ones from `$HOME/.grok/scripts/detect-commands.sh` (JSON from `Makefile` targets and `package.json` scripts) and add anything only `AGENTS.md`, `CLAUDE.md`, or `README.md` prose names; use `none` if unavailable. The executor chooses a proportional set under step 6 of the Fix work contract.
...
Do not interrupt to ask for missing branch, plan, report, or verification metadata; pass `none` where it is unavailable. Use `ask_user_question` only when the reported defect itself or its expected behavior cannot be reasonably inferred.
...
If the `spawn_subagent` tool or compatible sub-agent capability is unavailable, or dispatch fails, stop before editing. Report `Delegation status: unavailable` or `failed`, include the observed cause, and use `ask_user_question` to ask whether to continue with a direct main-session fix or stop. Direct execution is allowed only after the user explicitly chooses that fallback.
