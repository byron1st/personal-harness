...
Run this controller in a **`grok-4.6` / medium** session. T1 agents keep their own frontmatter pins (`high`). SuperGrok subscription quota still benefits from fewer rounds — prefer the lightest mode that fits the work; **unspecified invocations default to `light`**.
