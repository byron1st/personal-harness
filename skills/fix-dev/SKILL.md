...
A surgical fix-it skill for issues caught during a review gate. The executor diagnoses and patches one defect, then returns a summary. Implementation noise — file reads, grep hits, diff iteration, test output — stays with the executor.

This skill is methodology. It does not start a persona. Standalone (`/fix-dev`, no loop): the current session organises the brief and runs the work contract in place. Under `dev-loop`: the loop organises the brief, starts the `fixer` persona as a blocking child, and that persona follows this skill.
...
The user-facing input is only the defect, expected behavior, and any known pointer or review finding. Do not make workflow metadata a prerequisite. The caller (standalone session, or `dev-loop`) makes only lightweight read-only lookups needed to give the executor a short brief. The brief must contain:
...
- **Loop context** — optional: when started from a `dev-loop` run, the remediation round number and the LOOP file's absolute path; use `none` otherwise.
- **Workspace context** — capture `git status --short` before the run, then include the current branch and any pre-existing changes in known target files; gather this automatically.
...
- **Verification candidates** — known reproduction/targeted lint/test commands and project-required fast gates. If the caller brief already lists them, use those. Otherwise get the declared ones from `$HOME/.agents/scripts/detect-commands.sh` (JSON from `Makefile` targets and `package.json` scripts) and add anything only `AGENTS.md`, `CLAUDE.md`, or `README.md` prose names; use `none` if unavailable. The executor chooses a proportional set under step 6 of the Fix work contract.
...
Do not interrupt to ask for missing branch, plan, report, or verification metadata; pass `none` where it is unavailable. Ask the user only when the reported defect itself or its expected behavior cannot be reasonably inferred.

The loop starts one `fixer` persona per finding, sequentially. The persona has no access to the caller's conversation, so the brief must be **self-contained**. Standalone skips the persona and runs the work contract in the current session.

The caller never reads the touched files back into its own context after a loop executor returns. That defeats the purpose of isolating the fix.

## Present the result

When the executor returns, present the summary to the user roughly verbatim — root cause, files changed, verification outcomes, and any notes. Translate to Korean if the executor returned in English; keep file paths, command names, and code identifiers in their original form. Do not embellish with details the executor did not provide.

## Scope-guard handling

If the executor returns status `needs-confirmation` (it judged the fix out of scope after reading the code), **do not start a follow-up persona**. Show the user the explanation and rough implementation sketch, then route the work through `plan-dev`. Do not continue it within `fix-dev`; wait for the user to start or approve the planning step.

`needs-confirmation` and `blocked` are not retried. This skill does not retry `failed` under a different model. The loop may start the same persona once more; that retry is the loop's, not this skill's.

## Fix work contract

Follow every applicable step unless a stated `blocked` or `needs-confirmation` exit applies.

1. **Read inputs and protect the worktree** — capture `git status --short` before touching files. For every file you will edit that is already changed in that snapshot, inspect its staged and unstaged diff first. Never reset, checkout, discard, or overwrite a pre-existing change. Open the plan file(s) only when the brief provides them, then open the files pointed at by the brief. Read `AGENTS.md` / `CLAUDE.md` at the repo root (and any nested copies relevant to the changed files) for project conventions.
...
Return a **single concise message** that leads with the common stage block, then the fields below. The full diff lives on disk; do not paste it into the return. Do not include raw test output or grep dumps — summarise. Standalone uses the same shape in the final response.
...
The executor must **not** dump file contents, full diffs, or full test output into the return.

## Anti-patterns

- **Do not re-read the changed files in the caller "just to be sure"** — that defeats isolating the fix in a persona.
...
- **Do not create a new branch** — the caller's branch is the right one. Branch creation belongs to `implement-dev`, not here.
