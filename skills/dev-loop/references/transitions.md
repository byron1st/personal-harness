...
| IMPLEMENTING | implement-dev | `needs-design-decision` | all | start `plan-consultant` read-only → resume `implementer` with the decision in the brief. Does not consume loop budget. Not `blocked`. |
| IMPLEMENTING | implement-dev | `blocked` | all | BLOCKED_DIRECTION — surface `## Decision Needed`, suggest `plan-dev` re-entry |
| IMPLEMENTING | implement-dev | `failed` | all | start the same `implementer` once more; if still `failed` → ESCALATED |
...
| TESTING | test-dev | `failed` | all | start the same `tester` once more; if still `failed` → ESCALATED |
...
| FIXING | fix-dev | `failed` | all | start the same `fixer` once more; if still `failed` → ESCALATED |
...
If a persona cannot start, ask the user whether to run this stage in the current session or stop. Do not silently continue in place. That question is not a table row; it applies to every state that starts a persona.
...
- **Immediate**: `blocked`, `needs-confirmation`, or an unresolved `## Decision Needed` from any stage, per the table above. `failed` after the same-persona one-retry is also immediate. `needs-design-decision` is not an escalation.
