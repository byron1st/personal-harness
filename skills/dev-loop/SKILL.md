...
A thin controller that drives one approved **single-step** plan through implement → test → (`review-code` when the mode has review), with `fix-dev` remediation cycles — until every termination predicate holds, then stops at **READY_TO_COMMIT** for the human.

The loop owns **when**: the state machine, which persona this stage starts, human gates, and spawn-failure questions. It implements nothing itself and never merges the stage skills into one.

> The loop starts the stage's **persona**. The skill does not start a persona.
...
The loop starts a blocking child persona for each stage. That persona follows the named skill. LOOP log lines stay skill names (`implement-dev: pass`); do not rename them to personas.

| State | Persona the loop starts | Skill that persona follows |
| --- | --- | --- |
| IMPLEMENTING | `implementer` (one) | `implement-dev` |
| TESTING | `tester` (one) | `test-dev` |
| REVIEWING | the mode's axis set, in parallel | each follows the executor section of `review-code`; the loop follows the caller section |
| FIXING | `fixer` (one per finding, sequential) | `fix-dev` |

Personas do not start children. On a `(design-bearing)` fork the implementer returns `needs-design-decision`; the loop starts `plan-consultant`.

When starting `tester` in `light` or `noreview`, the brief must say **mutation is out of scope for this run**. When starting REVIEWING, the brief must name the axis set for this mode. Do not leave axis selection to description matching.
...
Each stage starts the persona in the table. The loop reads `## Stage Status` — plus `## Findings`, `## Evidence`, `## Decision Needed`, and `## Design Decision Needed` — and transitions per the table. Statuses the loop consumes: `pass | pass-with-suspected-defects | blocked | failed | needs-confirmation | needs-decision | changes-required | needs-design-decision`.

Human gates (Fix/Accept, READY_TO_COMMIT) stay in this session. A persona does not ask the user.

## Spawn failure

If a persona cannot start, ask the user whether to run this stage in the current session or stop. Do not silently continue in place. Direct in-place execution of a stage is allowed only after that explicit choice.

## `failed` retry

When an executor returns `failed` (hit the 3-attempts wall), start the **same persona** once more with the same brief plus one line naming what the first attempt tried and observed. If the second return is also `failed`, ESCALATED. Do not name a host model. `blocked`, `needs-confirmation`, and `needs-design-decision` are not retried this way.

## `needs-design-decision`

On IMPLEMENTING `needs-design-decision`, start `plan-consultant` read-only with the fork brief from `## Design Decision Needed`. Collect the short decision, then resume `implementer` with that decision in the brief. This does not consume loop budget. It is not `blocked`.
...
- **Multiple Fix findings in one round**: one `fixer` start per finding (its brief carries `Finding ID` and `Loop context`), sequentially. The round's TESTING / REVIEWING re-entry happens once, after all fixes.
...
- Each skill's internal 3-attempts-per-error rule stays internal. The loop's only retry of `failed` is the same-persona once rule above.
