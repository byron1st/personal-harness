...
2. Dispatch one implementation subagent with OpenCode's Task tool (`subagent_type: general`) and instruct it to use `implement-dev` on the plan path.
3. Wait for the implementation subagent to finish and parse its structured summary.
4. Dispatch one test-hardening subagent with OpenCode's Task tool (`subagent_type: general`) and instruct it to use `test-dev`, scoped to the implementation output.
5. Wait for the test subagent to finish and parse its structured summary.
6. Dispatch one review subagent with OpenCode's Task tool (`subagent_type: general`) and instruct it to use `review-code` with delegated reviewer agents against the final diff.
