...
Run the three phases in order — unit gaps, then e2e gaps, then mutation LIVED elimination — per the skill. Add only test code; never edit production/business logic (Global Rule 6). Do not run review-code. Do not revert edits made by others. Follow the repository's AGENTS.md and legacy CLAUDE.md when present, plus README.md / Makefile instructions.
