...
Adapt freely. For a small initiative you might skip "Architecture Overview" and "Tech Stack" if everything is already covered in `AGENTS.md` and legacy `CLAUDE.md` when present. For a SPEC.md-driven new project, "Requirements Coverage" is essential. The Mermaid DAG is optional but pays off when steps have non-trivial dependencies.
