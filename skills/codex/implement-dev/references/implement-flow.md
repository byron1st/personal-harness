...
- **Consultable** - a band *between* the two above: both candidate approaches are consistent with the plan, the plan is silent on which, and the wrong one is expensive to reverse (a persisted shape, a public signature, a concurrency model, a boundary other code will grow against). The *what* is not in question, so this is not direction-level; but the choice outlives the TODO, so treating it as ordinary mechanics is how a plan gets silently narrowed. **Only on a TODO tagged `(design-bearing)`**: spawn the `plan-consultant` custom agent with `fork_turns="none"` (read-only, T1; fall back to `explorer` carrying the consultant contract if unavailable) for a short decision, record the decision and its rationale under the TODO's `편차` line, and continue. Consultation resolves the fork - it does not pause the run and does not need the user.
  ...
  - **A consultant cannot authorize a direction change.** If the answer would contradict the plan's goal / approach / `## Key decisions` / `## Non-goals`, the item was direction-level all along: stop and escalate per the direction-level bucket. Do not use consultation to route around the user.
  - **Worker mode only.** In interactive execution the user is already present — ask them.
