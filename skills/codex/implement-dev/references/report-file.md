...
- The cockpit sections (`## Summary`, `## Review Map`, `## Red Flags`, `## Open Questions`, `## Change Walkthrough`) are the high-signal block the skill prints to the session. When printing `## Change Walkthrough`, convert each saved `path:line` anchor into a clickable Markdown file link with the repo-relative path as the label and the absolute path plus line number as the target, e.g. `[src/app.ts:42](/absolute/path/to/repo/src/app.ts:42)`. Everything below `## Change Walkthrough` is detail the reviewer reads on demand from the file.
