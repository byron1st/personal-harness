...
description: "Drive an approved single-step plan through implement/test only (no review, no mutation) until READY_TO_COMMIT. Optional lightest loop; Codex default is dev-loop-light. Requires Acceptance Contract + Authority Boundaries."
---
...
This is the Claude default. Sibling variants: [`dev-loop-light`](../dev-loop-light/SKILL.md) adds two review axes, [`dev-loop`](../dev-loop/SKILL.md) adds four plus mutation. Pick before starting; the loop does not switch variants mid-run.
