---
name: dev-loop
description: "Drive an approved single-step plan through the autonomous implement/test/review/(triage-fix) loop until READY_TO_COMMIT, checkpointing to a LOOP file. Use when asked to run a saved plan end to end as a loop ('루프 돌려', 'dev-loop 실행'). Requires Acceptance Contract + Authority Boundaries."
---
...
4. Create the LOOP file — `docs/agents/dev/{plan stem with _PLAN_ → _LOOP_}.md` per [references/loop-state.md](references/loop-state.md). If it already exists for this plan, **resume** instead: trust the file over memory, continue from the last round's `Next`, and never rewrite prior rounds.
...
The loop never: commits, pushes, or creates PR/MRs; writes an AR entry on its own (only the user's explicit Accept in triage does); weakens tests; auto-fixes `[NORMAL]`/`[LOW]` findings; merges stage skills or dispatches their Workers directly; orchestrates control flow through hooks; exceeds or self-overrides the loop budget; changes plan direction — that requires `plan-dev` re-entry.
