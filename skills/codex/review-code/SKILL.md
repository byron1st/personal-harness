...
2. **Ask for a text classification**: Codex has no structured question tool — after the table, ask the user to reply per ID in the form `REVIEW-001: fix` or `REVIEW-001: accept`. Ambiguous replies (format mismatch, unknown id, words other than fix/accept) are re-confirmed, never guessed. An ID with no reply stays **unclassified** — never auto-accept, and never auto-classify to Fix; classification is fixed only by the user's explicit reply.
