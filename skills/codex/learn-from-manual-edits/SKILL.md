...
Maintain a dedicated section in the project root `AGENTS.md` — the instructions file Codex reads automatically; if the project keeps its conventions in `CLAUDE.md` only, use that file instead (create the file and/or section if missing, appended at the end):
