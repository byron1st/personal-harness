...
`test-dev`'s **Dispatch mode** (the main session, default) delegates the actual test hardening to a single `tester` subagent (the **Worker**, spawned as the custom `tester` agent with `fork_turns="none"`; a built-in `worker` given this contract in full is the fallback when that persona is unavailable). This file is the **single source of truth** for that delegation: the prompt the dispatcher hands the Worker, the fixed-heading return the Worker must emit (②), and the chat summary the dispatcher owes the user (③).
