---
name: plan-dev
description: Create an implementation plan in Cursor plan mode and persist plan/research artifacts under docs/agents after approval. Use when the user wants to plan before coding; single-step by default, multi-step only on explicit request.
---

# Plan Dev

Create an implementation plan through iterative refinement entirely inside Cursor plan mode, then persist the result as markdown file(s) under the project root's `docs/agents` directory as the very first action after Cursor transitions out of plan mode.
...
Steps 1-9 below MUST run with read-only tools only (file reads, `rg`/file searches, user questions, direct file search under the project root). No writes to `docs/agents` or the working tree happen during this phase.

When the user builds the plan, Cursor exits plan mode through the UI's build flow. There is no agent-callable plan-exit tool in Cursor.

The first tool calls after the user has approved/built the plan and Cursor is allowed to write MUST be the persistence steps in step 11, in this exact order: research files -> plan file(s). Only after those writes are done may any further follow-up work begin.
