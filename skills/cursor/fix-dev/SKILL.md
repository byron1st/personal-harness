---
name: fix-dev
description: Fix a bug found during review or verification after implementation. Use when the user asks to correct a defect without a new plan; delegate to a Cursor subagent only when explicitly requested.
---
...
The main session is the only place that has the surrounding context — which plan, which step, what was just reviewed, what the user reported. Distil this into a short brief so the executor does not have to guess. The brief must include:
...
When a subagent is used, the main session never reads the touched files back into its own context after the subagent returns. That defeats the purpose of delegation.

### 3. Present the result

When a subagent returns, present the summary to the user roughly verbatim — root cause, files changed, verification outcomes, and any notes. Translate to Korean if the subagent returned in English; keep file paths, command names, and code identifiers in their original form. Do not embellish with details the subagent did not provide. When the main session performed the fix, present the same fields directly.

### 4. Scope-guard handling

If the executor returns status `needs-confirmation` (it judged the fix out of scope after reading the code), **do not dispatch a follow-up automatically**. Show the user the explanation of why the fix exceeds the boundary, and ask whether to:
...
When using a subagent, its prompt must specify the following sequence. When not delegating, the main session follows the same sequence directly. Steps 3 and 4 are the only ones that branch on outcome; everything else runs unconditionally.
...
- **Do not re-read the changed files in the main session "just to be sure" after a subagent returns** — that defeats the entire purpose of delegation.
