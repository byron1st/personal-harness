# Test-hardening delegation contract

`test-dev` uses this contract only when the user explicitly asks for delegation. In that opt-in path, the main session acts as the Dispatcher and hands the actual test hardening to a single delegated subagent via Cursor's Task tool. This file is the **single source of truth** for that delegation: the prompt the dispatcher hands the Delegate, the fixed-heading return the Delegate must emit (②), and the chat summary the dispatcher owes the user (③).

`test-dev` references this contract instead of restating it. Do not duplicate these templates elsewhere; update them here. This contract does not change the Cursor variant's default interactive behavior when delegation was not requested.

## A. Delegate signal

The Delegate detects it is the delegated subagent (not the main session) from the dispatch prompt itself: the prompt contains the line `You are running as the test-hardening Delegate subagent.` A session that sees this line runs the three-phase flow directly and **does not re-dispatch** another delegated subagent - that would double-nest. A session that does not see this line stays in the main-session flow; it dispatches exactly one Delegate only when the user explicitly requested delegation, subagent work, or parallel agent work.

## B. Dispatch prompt

The dispatcher (the main `test-dev` session) packages the brief below into a self-contained prompt and hands it to one Delegate via Cursor's Task tool. Scope is defined by the diff — the implementation report, if passed, is only an intent hint:

```text
You are running as the test-hardening Delegate subagent.
...
When done, return only the fixed-heading Markdown in section C of this contract - do not summarise the `## Suspected Business Logic Defects` list, return it verbatim.
```

## C. Delegate return message (②)

Fixed `##` headings, Markdown. The dispatcher parses these headings by name, so the Delegate must emit them **exactly** and must not invent or rename headings. Anything empty becomes `none` (or its bullets, `none`); never drop a heading.
...
Translate to Korean if the Delegate returned English; keep paths, command names, and code identifiers in their original form.
