...
`test-dev` runs in one of two delegated modes, detected from the invoking prompt (the **delegate signal**). Under this Cursor variant's policy, subagents are spawned only when the user explicitly asks for delegation, subagent work, or parallel agent work.

- **Delegate (delegation, subagent — opt-in)** — a session invoked with `You are running as the test-hardening Delegate subagent.` in its prompt. It runs the three-phase flow directly against the scope handed to it, does not re-dispatch, and returns the fixed-heading Markdown from the delegation contract reference under `references/`. The Dispatcher hands the Delegate a self-contained prompt and dispatches it via Cursor's Task tool; the Delegate starts cold.
- **Dispatcher (opt-in, main session)** — when the user explicitly requests delegation, the main `test-dev` session acts as the Dispatcher. The Dispatcher does **not** edit tests itself; it resolves the review scope ([Determine Scope](#determine-scope)), gathers verification commands and conventions once, then launches exactly **one** Delegate using the prompt, return schema, and chat-summary shape in the delegation contract reference under `references/`, and renders a short chat summary from the Delegate's fixed-heading return. The Dispatcher does not re-dispatch another Delegate once one is running.

Interactive (the **default** under this skill policy): when a session runs `test-dev` directly in the main session without the user requesting delegation (or when subagent dispatch is not available on the host), the three-phase flow runs in-place. Follow the Delegate rules except that a blocking obstacle (missing verification command, absent mutation tooling) goes back to the user interactively rather than being returned as `blocked`. A user simply invoking `test-dev` from the main chat gets interactive direct execution; Delegate dispatch requires explicit opt-in.
...
The **Dispatcher** (or the interactive main session) resolves the scope in git terms **before dispatching** — the same grammar as `review-code`. The Delegate never re-derives scope from conversation; it receives the resolved scope in its prompt.
...
If an `implement-dev` report exists for the change, the Dispatcher may pass its path to the Delegate as an **optional intent hint** — never as the scope definition. Scope is always the git diff. Keeping the Delegate's brief lean (diff + conventions, not the author's narrative) preserves the fresh-eyes advantage that makes gap analysis worth isolating.

## Prepare

1. **Verification commands**: extract lint, unit, e2e, and mutation commands from `Makefile`, `AGENTS.md`, `CLAUDE.md`, or `README.md`. The Dispatcher resolves any missing required command before dispatch; a Delegate that still finds one missing returns `blocked` with `## Decision Needed` (interactive execution asks the user).
...
3. **Mutation tooling**: find the project's mutation target (typical: `make test-mutation`). If no tooling is configured, the Dispatcher decides before dispatch; a Delegate returns `blocked` with the options (skip Phase 3 / nominate a command / install a standard tool), and interactive execution asks the user.
...
If a pre-existing test is red and reverting recent test changes does not restore it, stop and surface the failure with what was tried — Delegate mode returns `## Test Status: failed`; interactive execution surfaces it to the user.

## Output

The result is delivered as two artifacts, defined in the delegation contract reference under `references/`:

- **Delegate return (②)** — the fixed-heading Markdown the Delegate hands back (`## Test Status` … `## Remaining Attention Items`, plus `## Decision Needed` when blocked). No file artifact is written, so the return **is** the deliverable: the Delegate must emit the `## Suspected Business Logic Defects` list **verbatim**, never summarised or dropped.
- **Dispatcher chat summary (③)** — the Dispatcher renders a short summary for the user: scope, per-phase one-liners (unit added/result, e2e added/result or skipped, mutation start→final efficacy and whether the 80% threshold was reached), and the suspected-defects list surfaced **prominently and verbatim** as the most important attention item. If the Delegate returns `blocked`, surface `## Decision Needed` first and stop. Translate to Korean if the Delegate returned English; keep paths and identifiers as-is.
...
6. **Stop after 3 failed attempts on the same error** — record what was tried, what was observed, and add it to the defect list. Continue to the next gap; the user gets the full picture in the final summary. If the error is irrecoverable or a previously-green test cannot be restored, Delegate mode sets `## Test Status` to `failed` and returns; interactive execution asks the user for guidance. Suspected business-logic defects are never a `failed` condition — they go to the defect list and are reported.
