...
4. **Use the `AskQuestion` tool for multiple-choice questions.** When the question has 2–4 concrete options (tech stack, architecture pattern, etc.), invoke `AskQuestion` so the developer can pick from a structured UI. For open-ended questions (purpose, business rules, free-form descriptions), ask in plain text.
...
Generate the complete SPEC.md following the template below. In the editor:
...
2. For any feedback, apply changes with the `Edit` tool — modify only the affected sections, not the whole file.
