...
4. Use structured user input for multiple-choice questions when the current Cursor mode provides it. When no structured input tool is available, ask in plain text with numbered options. For open-ended questions such as purpose, business rules, or free-form descriptions, ask in plain text.
