...
The dispatcher (the main `implement-dev` session) hands the Worker this prompt, replacing placeholders. It fills the verification-command block from `Prepare` step 2 (`$HOME/.cursor/scripts/detect-commands.sh` plus any prose-only commands) — passing them is the point: without this block the Worker rediscovers the same commands cold on every dispatch.
