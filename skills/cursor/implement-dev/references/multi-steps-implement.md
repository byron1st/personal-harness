...
1. **Run** step N:
   - Delegation explicitly requested: dispatch a Cursor subagent via the Task tool (section 4). Wait for it to return.
...
Use this section only when the user explicitly requested subagents, delegation, or parallel agent work. Spawn a Cursor subagent using the Task tool with a self-contained prompt. Do not fork the parent conversation unless the user explicitly asked for that; the prompt must include every input the subagent needs. The subagent starts in a clean context and runs with the parent session's permissions; do not assume it has independent permissions.
