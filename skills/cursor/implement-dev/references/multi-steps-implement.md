...
- `## Conventions` / `## Tech Stack` / `## Architecture Overview` — context every step must respect; pass these to every subagent when delegating, or keep them in view when working in the main session.
...
1. **Run** step N:
   - Delegation explicitly requested: dispatch a Cursor subagent (section 4). Wait for it to return.
...
3. **Wait** for explicit user approval. If the user requests changes, use a follow-up `subagent` only when delegation was explicitly requested; otherwise make the follow-up changes in the main session. Re-request approval and do not merge until approved.
...
Use this section only when the user explicitly requested subagents, delegation, or parallel agent work. Spawn a Cursor subagent with a self-contained prompt. Do not fork the parent conversation unless the user explicitly asked for that; the prompt must include every input the subagent needs. The subagent runs under the same permissions as the parent session, so do not assume it has independent access.
...
The full report content lives in Obsidian; the subagent must **not** dump the entire report into its return message.

### 4.3 Follow-up dispatch when the user requests changes

If the user asks for changes after reviewing the report and delegation was explicitly requested, dispatch a follow-up `subagent`. The follow-up prompt mirrors section 4.1 plus:
...
## 5. Per-step work [subagent/main session]

The subagent or main session performs the following sequence and produces the summary defined in section 4.2.
...
If running in a subagent, return the summary defined in section 4.2 as the subagent's single final message. If running in the main session, prepare the same fields as the step summary for the review gate.
...
If this fails, the merge introduced a regression. Investigate; for non-trivial fixes, use a fresh subagent on a `hotfix/step-N` branch only when delegation was explicitly requested. Otherwise fix in the main session.
