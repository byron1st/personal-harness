---
name: implement-dev
description: Execute a plan-dev implementation plan with TDD, verification, TODO updates, and repository-local implementation reports under .agents/doc. Use when the user asks to implement a saved single-step or multi-step plan.
---
...
- If the user explicitly requested delegation for this implementation run, the main session spawns one Cursor subagent via the Task tool per step, sequentially. The subagent runs the per-step build: branch creation, sub-plan TDD work, build verification, completion report, commit, and returns a concise summary.
...
Do not spawn a subagent just because the plan is multi-steps. The Cursor delegation path and the main-session fallback are defined in [references/multi-steps-implement.md](references/multi-steps-implement.md).

### 4. Multi-steps: per-step review gate (no auto-merge)

Steps are executed **one at a time, sequentially**. After step N is complete:

1. The completion report (including a `## Manual Verification` checklist) has been written under `.agents/doc/dev`.
