...
After saving the report file, print its **review cockpit** — the `## Summary`, `## Review Map`, `## Red Flags`, `## Open Questions`, and `## Change Walkthrough` sections, verbatim — as the session's final output, followed by the report's path. The cockpit is the high-signal block that directs the user's review; the `## Key Decisions` and everything below it stay in the file for on-demand reading, so do not print them. Always include the path so the user can open the full report.
